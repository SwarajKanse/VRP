...
class RouteCanvas(ttk.Frame):
    ...
    def __init__(self, master, road_service: RoadService, path_mode: str = "road"):
        ...
        self._pin_image: tk.PhotoImage | None = None
        ...
        self.canvas.bind("<Leave>", lambda _event: self._hide_pin_marker())
    ...
    def set_pin_cursor(self, image: tk.PhotoImage | None, enabled: bool) -> None:
        self._pin_image = image
        self._pin_enabled = enabled and image is not None
        self.canvas.configure(cursor="none" if self._pin_enabled else "fleur")
        if not self._pin_enabled:
            self._hide_pin_marker()
    ...
    def _draw_now(self) -> None:
        ...
        if self._nodes:
            ...
            self.canvas.create_text(
                width / 2,
                height / 2,
                text="Add depots and orders to begin.",
                fill="#6b7280",
    ...
    def _animate(self) -> None:
        ...
        for item in self._animation_routes:
            ...
            item["progress"] = min(1.0, active_time / item["duration_seconds"])
    ...
    def _handle_drag(self, event) -> None:
        if self._pin_enabled:
            self._draw_pin_marker(event.x, event.y)
            return
    ...
    def _handle_release(self, event) -> None:
        if self._pin_enabled:
            if self._click_callback is not None:
                self._click_callback(self._screen_to_geo(event.x, event.y), None)
            return
        if self._drag_origin is None:
            return
    ...
    def _draw_pin_marker(self, x: float, y: float) -> None:
        if not self._pin_enabled or self._pin_image is None:
            return
        if self._pin_marker is None:
            self._pin_marker = self.canvas.create_image(x, y, image=self._pin_image, anchor="s", tags="pin")
            return
        self.canvas.coords(self._pin_marker, x, y)

    def _hide_pin_marker(self) -> None:
        if self._pin_marker is not None:
            self.canvas.delete(self._pin_marker)
            self._pin_marker = None
